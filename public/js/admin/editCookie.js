...
async function editCookieForm() {
  ...
  await loadCookieData(id);

  async function loadCookieData(id) {
    try {
      const response = await fetch(`${backendAPI}cookie/${id}`, {
      ...
      if (data.success) {
        document.getElementById('title').value = data.cookie.title;
        CKEDITOR.instances.text.setData(data.cookie.text);
  ...
  function showError(message) {
    ...
    document.getElementById('cookie-form').appendChild(errorDiv);
    setTimeout(() => errorDiv.remove(), 5000);
  }

  // Validación del formulario
  document
    .getElementById('cookie-form')
  ...
  async function submitForm() {
    ...
    try {
      ...
      const response = await fetch(`${backendAPI}edit-cookie/${id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      if (data.success) {
        ...
        showError(data.message || 'Error al editar la cookie');
      }
    } catch (error) {
