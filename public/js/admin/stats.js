...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function getUserStats() {
    ...
    if (ordersData) {
      ...
      ).textContent = `${totalSubscriptions.toFixed(2)} €`;
      ...
      document.getElementById('ppv-amount').textContent = `${totalPPV.toFixed(
        2
      )} €`;
    }
  }
  ...
  function renderPlansChart(users, admins, plansData) {
    ...
    // Inicializar contadores para planes existentes
    ...
    const NO_PLAN_ID = 'no_plan';
    planCounts[NO_PLAN_ID] = 0;
    planColors[NO_PLAN_ID] = '#CCCCCC'; // Color gris para usuarios sin plan
    planNames[NO_PLAN_ID] = 'Sin plan';

    // Contar usuarios por plan y sin plan
    users.forEach((user) => {
      if (user.plan && user.plan.id) {
        planCounts[user.plan.id]++;
      } else {
        planCounts[NO_PLAN_ID]++;
      }
    });

    // Filtrar planes con al menos un usuario (incluyendo "Sin plan")
    const labels = [];
    const data = [];
    ...
    legendItems.forEach((item) => {
      ...
      text.textContent = `${item.name}: ${item.count} usuarios (${(
        (item.count / (users.length - admins)) *
        100
      ).toFixed(1)}%)`;
